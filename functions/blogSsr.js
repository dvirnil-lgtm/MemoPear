...
// Metadata for the built-in blog posts that ship in the app build
// (components/Blog.tsx). Generated by scripts/gen-static-blog-posts.mjs. These
// posts render via the static prerendered fallback (see nginx.conf) even when
// they were never seeded into Firestore through the CMS, so they must also
// appear in the sitemap and llms.txt. Merged with the live Firestore posts by
// slug in loadAllPublicPosts().
let STATIC_BLOG_POSTS = [];
try {
  STATIC_BLOG_POSTS = require('./staticBlogPosts.json');
} catch (_) {
  // Missing generated file — degrade to Firestore-only rather than crash.
  STATIC_BLOG_POSTS = [];
}
...
// Every public blog URL: the live Firestore posts unioned with the built-in
// static posts (from staticBlogPosts.json), deduplicated by slug. Firestore
// wins on conflict, since a post edited in the CMS is the current version.
// Used for the sitemap and llms.txt so no published blog is ever missing,
// whether it lives in Firestore, in the static build, or both.
async function loadAllPublicPosts(db) {
  const live = await loadPublishedPosts(db);
...
function buildSitemap(posts) {
  ...
  const urls = staticRoutes
    ...
    .concat(
      posts.map(
        (p) => `  <url>\n    <loc>${SITE_URL}/blog/${escapeHtml(p.slug)}</loc>\n    <lastmod>${new Date(p.updatedAt || p.date || Date.now()).toISOString().slice(0, 10)}</lastmod>\n    <changefreq>monthly</changefreq>\n    <priority>0.7</priority>\n  </url>`,
      ),
    )
    .join('\n');
...
function createBlogSsrHandler(db) {
  return async function handler(req, res) {
    ...
    try {
      // Dynamic sitemap (live Firestore posts + built-in static posts).
      if (path === '/sitemap.xml') {
        const posts = await loadAllPublicPosts(db);
        res.set('Content-Type', 'application/xml; charset=utf-8');
        res.set('Cache-Control', 'public, max-age=300, s-maxage=600');
        return res.status(200).send(buildSitemap(posts));
      }

      // Dynamic llms.txt (live Firestore posts + built-in static posts).
      if (path === '/llms.txt') {
        const posts = await loadAllPublicPosts(db);
